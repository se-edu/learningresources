...
* **Do include a table of contents.** A table of contents tells the reader what to expect, such as how long the article is or what content is covered. It serves as a reference that makes it easier to jump around from section to section. In our case, we only show the table of contents for mobile users and for printing as Markbind already provides a navigation panel on the right side of every article. #How about link to specific code snippet on github?# If you have programming experience, feel free to refer to [this]() to see how to implement this behavior. Otherwise, do include a table of contents in the document you send us.
