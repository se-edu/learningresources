...
An example of a task is machine translation, where text is converted from one language to another by a computer. It is difficult to write clear instructions for machine translation with regular programming. State of the art [machine translation algorithms](https://arxiv.org/abs/1706.03762) leave the instruction generation to ML algorithms.

The ML algorithms are fed with data to approximate the ideal instructions for performing the task. These approximations are never 100% correct and there are various parameters to tweak for maximizing performance. 

Statistical analysis is used to evaluate the performance of different ML algorithms and configurations. 
...
#### Supervised Learning
The data used for training is accompanied with desired outputs for the task. For the case of machine translation, an example input is a sentence in the source language. The desired output is the corresponding sentence in the target language.   
...
<img src="https://i.imgur.com/mKjIS0C.png" width="450">  
...
In the case of classification, the data is grouped under labels generated by the algorithm. The down side is the difficulty in explaining the relationship between items in a group.
...
<img src="https://cdn-images-1.medium.com/max/900/1*xTvsgpDfja05SRMt-H5ylA.png" width="450">  
...
The FastAI ML MOOC[1] is a great source of information on data partitioning. 
...
Guidelines on picking a size for the validation set can be found in Lesson 7 of FastAI's ML MOOC[1]. 

The following driving factors are suggested for deciding the validation set size.  

- Business Concerns   
Business concerns determine the significance of mistakes in predictions. For example, 4 mistakes in fraud detection is a much bigger issue than 4 mistakes in [cucumber classification](https://cloud.google.com/blog/big-data/2016/08/how-a-japanese-cucumber-farmer-is-using-deep-learning-and-tensorflow). A large enough validation set tells us if the difference in performance is significant enough for the given problem. 
...
An example of data analysis for convolutional neural networks (an emerging class of ML algorithms) is to observe where the neural network is looking at when it makes its predictions. This can be done with class activation maps. The practitioner may choose to perform further data preprocessing to help the neural network focus on the right areas. 
